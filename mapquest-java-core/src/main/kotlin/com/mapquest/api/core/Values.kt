...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { MapquestInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw MapquestInvalidDataException("`$name` is not set")
            is JsonNull -> throw MapquestInvalidDataException("`$name` is null")
            else -> throw MapquestInvalidDataException("`$name` is invalid, received $this")
        }
